...
if not st.session_state.user_id:
    ...
    login_tab, reg_tab = st.tabs(["🔑 เข้าสู่ระบบ", "✨ สมัครสมาชิกใหม่"])
        ...
        with st.form("register_form"):
            ...
            if st.form_submit_button("สมัครสมาชิก", use_container_width=True):
                if rg_pw != rg_pw2:
                    ...
                    ok, msg = register_user(rg_user, rg_pw)
                    if ok:
    ...
    st.caption("💡 ลืมรหัสผ่าน? ติดต่อผู้ดูแล LINE: 0610950531")
    st.stop()
...
with st.sidebar:
    ...
    # ===== Admin Panel (เฉพาะเจ้าของเว็บ — ล็อกด้วยรหัส) =====
    st.divider()
    with st.expander("🛠️ สำหรับผู้ดูแลระบบ"):
        # อ่านรหัส admin จาก secrets ถ้ามี ไม่งั้นใช้ค่าเริ่มต้น
        try:
            _admin_pw = st.secrets.get("ADMIN_PASSWORD", "admin1234")
        except Exception:
            _admin_pw = "admin1234"
        admin_input = st.text_input("รหัสผู้ดูแล", type="password", key="admin_pw_input")
        if admin_input:
            if admin_input == _admin_pw:
                st.session_state.is_admin = True
                st.success("✅ เข้าสู่โหมดผู้ดูแล")
            else:
